...
static struct field_stats debug_fields[] = {
    ...
    {"err1_result_TD", OUR(err1_result_TD), -1, 'B', 0,0,0, 0,0, 0},
    /* err2 debug fields — oracle offsets = struct offsets (packed struct) */
    {"err2_delay_revised_value", OUR(err2_delay_revised_value), -1, 'd', 0,0,0, 0,0, 0},
    {"err2_delay_roc", OUR(err2_delay_roc), -1, 'd', 0,0,0, 0,0, 0},
    {"err2_delay_slope_sharp", OUR(err2_delay_slope_sharp), -1, 'd', 0,0,0, 0,0, 0},
    {"err2_delay_roc_cummax", OUR(err2_delay_roc_cummax), -1, 'd', 0,0,0, 0,0, 0},
    {"err2_delay_slope_cummax", OUR(err2_delay_slope_cummax), -1, 'd', 0,0,0, 0,0, 0},
    {"err2_delay_glu_cummax", OUR(err2_delay_glu_cummax), -1, 'd', 0,0,0, 0,0, 0},
    {"err2_delay_flag", OUR(err2_delay_flag), -1, 'B', 0,0,0, 0,0, 0},
    {"err2_cummax", OUR(err2_cummax), -1, 'd', 0,0,0, 0,0, 0},
    {"err2_crt_current[0]", OUR(err2_crt_current[0]), -1, 'B', 0,0,0, 0,0, 0},
    {"err2_crt_current[1]", OUR(err2_crt_current[1]), -1, 'B', 0,0,0, 0,0, 0},
    {"err2_crt_glu[0]", OUR(err2_crt_glu[0]), -1, 'B', 0,0,0, 0,0, 0},
    {"err2_crt_glu[1]", OUR(err2_crt_glu[1]), -1, 'B', 0,0,0, 0,0, 0},
    {"err2_condi[0]", OUR(err2_condi[0]), -1, 'B', 0,0,0, 0,0, 0},
    {"err2_condi[1]", OUR(err2_condi[1]), -1, 'B', 0,0,0, 0,0, 0},
    /* err4 debug fields */
    {"err4_min", OUR(err4_min), -1, 'd', 0,0,0, 0,0, 0},
    {"err4_range", OUR(err4_range), -1, 'd', 0,0,0, 0,0, 0},
    {"err4_min_diff", OUR(err4_min_diff), -1, 'd', 0,0,0, 0,0, 0},
    /* err16 debug fields */
    {"err16_CGM_plasma", OUR(err16_CGM_plasma), -1, 'd', 0,0,0, 0,0, 0},
    {"err16_CGM_ISF_smooth", OUR(err16_CGM_ISF_smooth), -1, 'd', 0,0,0, 0,0, 0},
    /* err128 */
    {"err128_flag", OUR(err128_flag), -1, 'B', 0,0,0, 0,0, 0},
    {"err128_revised_value", OUR(err128_revised_value), -1, 'd', 0,0,0, 0,0, 0},
    {"err128_normal", OUR(err128_normal), -1, 'd', 0,0,0, 0,0, 0},
...
int main(int argc, char **argv) {
    ...
    if (eapp_override != 0.0f) {
        ...
        if (strcmp(basename, "lot1") == 0) {
            dev_info.eapp = 0.15f;
            printf("Auto-detected lot1: eapp=0.15\n");
        } else if (strcmp(basename, "lot2") == 0) {
            dev_info.eapp = 0.05f;
