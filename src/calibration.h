/*
 * CareSens Air CGM Calibration — Data Structure Definitions
 *
 * Ported from Juggluco's air.hpp (GPL-3.0) to pure C.
 * All structs are packed to match the proprietary library's ABI.
 */
#ifndef CARESENS_CALIBRATION_H
#define CARESENS_CALIBRATION_H

#include <stdint.h>

/* Calibration log entry — uses natural ARM alignment (104 bytes per entry) */
...
/* Persistent algorithm state — uses natural ARM alignment (117312 bytes) */
struct air1_opcal4_arguments_t {
    ...
    double biasavg[2];
    double nSumtrend;
    uint16_t bias_flag;        /* 0x2B70: 0=inactive, 3=active correction */
    uint16_t bias_cnt;         /* 0x2B72: step counter within correction period */
    uint8_t _pad_bias[4];      /* 0x2B74: padding */
    double holt_level;         /* 0x2B78: Holt level component (f78) */
    double holt_forecast;      /* 0x2B80: Holt forecast = level + h*trend (f80) */
    double holt_trend;         /* 0x2B88: Holt trend component (f88) */
    double kalman_roc[4];
...
/* Debug oracle — ALL intermediate values (1579 bytes) */
...
/* Factory calibration parameters (from sensor BLE) */
...
/* User calibration list (passed EMPTY for factory-cal-only mode) */
...
/* Function signature — matches the proprietary library's export */
